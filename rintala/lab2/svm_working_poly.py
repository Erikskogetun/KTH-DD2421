...
def b_value():
	...
	for i in range(len(non_zero_dp)):
		b+=non_zero_a[i]*non_zero_t[i]*radial_kernel(non_zero_dp[i],s)
	b= b-st
	print("b:		",b)
...
def indicatorFunction_2(x,y):
	...
	dp = numpy.array([x,y])
	kernalValue = radial_kernel(non_zero_dp, dp)
	value = numpy.dot(non_zero_a, non_zero_t)
...
def indicatorFunction(x,y):
	...
	for i in range (len(non_zero_a)):
		sum+=non_zero_a[i]*non_zero_t[i]*radial_kernel(non_zero_dp[i],[x,y])
	sum2 = sum-b
	return sum2
...
def radial_kernel(dp1, dp2, sigma=8):
...
def calculatePMatrix():
	...
	for i in range(N):
		for j in range(N):
			PMatrix[i][j] = targets[i]*targets[j]*radial_kernel(inputs[i], inputs[j])
